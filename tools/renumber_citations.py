...
a correr con el mapa nuevo.
  ...
  python tools/renumber_citations.py --de-alfa-a-numero   migracion, una vez
...
def main():
    ...
    emit("mapa", " ".join("%s>%s" % (k, mapa[k]) for k in
         ...
         "de la clave alfabetica interna al numero publico")
    ...
    for nombre in FUENTES:
        ...
        alfa = re.findall(r"\[([A-Z][A-Za-z]{1,3})\]", prosa(t))
        huerfanas = sorted(set(a for a in alfa if a not in mapa))
        if huerfanas:
            FALLOS.append("%s: claves alfabeticas sin referencia: %s"
    ...
    emit("claves.alfabeticas.encontradas", total_alfa)
